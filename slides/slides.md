...
**ICs (Integrated Circuits, "semiconductor chips")** consist of millions  or even billions of field effect transistors (FETs) and other electronic components (resistances, capacities) that process information and perform various functions in electronic devices, such as computers.

## Fundamentals to ASIC design workflows

**ASICs and design workflows**

In contrast to *general-purpose chips* such as microprocessors, which can be used for a wide variety of tasks, **ASICs (application-specific integrated circuits)** are specifically designed for a specific purpose, such as AI accelerators, cars, and medical devices.
...
A characteristc output of an ASIC design flow is a **GDSII-File**, which allows the produktion of a *set of photolithographic masks*. These masks can be sent to a semiconductor fab for production..
...
Digital hardware is designed using Hardware Description Languages (HDLs) where developers describe the desired behaviour of the hardware design.

Most digital chips are designed in SystemVerilog or VHDL. SystemVerilog has syntax that is loosely inspired by C and a weak type system, while VHDL is insipired by Ada and has a much stronger type system.

For analog designs, there is Verilog-AMS and VHDL-AMS.

There are also more modern alternatives to Verilog and VHDL including Chisel, BlueSpec, Amaranth, SystemC, and Spade


## Fundamentals to ASIC design workflows

- **RTL Synthesis:** In this step, the *HDL description of the logic circuit*, at Register Transfer Level (RTL), i.e., the \*.v or \*.vhdl file, is converted into a circuit consisting of logic cells, the so-called *netlist*.

- **_Functional Verification:_** It is important to identify design errors early on. Therefore, the hardware description of the circuit, e.g., in Verilog, must verified against the circuit requirements. This is done through *simulation or formal methods*, both on the original HDL description and the *netlists generated by synthesis*.
...
**Fundamental phases of ASIC design workflows - Backend**
 ...
 The _netlist_ produced by RTL synthesis describes which logic cells are going to be used in the final chip, and how they are connected. However, it does not specify the physical location of the cells, or the wires that will connect them.

- **Layout Synthesis** is a complex sequence of steps including floor planning, placement, routing, and clock tree synthesis that convert the netlist into a physical layout that can be manufactured.

Because placement and routing are the most time-consuming processes, these steps are sometimes referred to collectively as **Placement and Rounting (PnR)**
