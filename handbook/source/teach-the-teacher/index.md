...
The educational approach adopted in this material is primarily based on the [Problem-Based Learning (PBL) method](https://mediapool.hm.edu/mediapool/media/dachmarke/dm_lokal/bologna/hd_mint/veroeffentlichungen_3/Wolf_et_al_HDMINT_2013.pdf). Learners are presented with realistic design challenges — such as synthesizing, simulating, and verifying a digital block — and guided to solve them using open-source EDA tools.
...
- Provide instructors with clear {doc}`learning_objectives` and {doc}`course_structure`.  
- Offer {doc}`suggested_problems` that can be adapted to different audiences.  
- Define {doc}`assessment_guidelines` that focus on understanding and problem-solving rather than rote tool execution.  
- Build a growing community of educators contributing to open hardware education.

## Structure

This chapter contains the following sections:

1. {doc}`learning_objectives` — defines conceptual, technical, and pedagogical goals.  
2. {doc}`course_structure` — 
3. {doc}`suggested_problems` — hands-on modules aligned with the main design flow.  
4. {doc}`assessment_guidelines` — evaluation rubrics and teaching strategies.  
