...
https://librelane.readthedocs.io/en/latest/getting_started/newcomers
...
In contrast to "general-purpose chips" such as microprocessors, which can be used for a wide variety of tasks, **ASICs (application-specific integrated circuits)** are specifically designed for a specific purpose, such as smartphones, cars, and medical devices. Smartphone processors, for example, focus on low power consumption and high performance for running apps and streaming media, and often include specialized cores for graphics processing and artificial intelligence.

Developing an ASIC is a complex process that encompasses numerous steps from idea to manufacturing data, requiring extensive expertise in chip design and can take several months.

Engineers can optimize this process in terms of content and time by using a proven ASIC design workflow, with each phase supported by powerful EDA (Electronic Design Automation) tools. Below you will find examples of the individual modules for implementing an ASIC:

- **Design Entry:** In this step, the logic circuit design is created using a hardware description language (HDL) (e.g., System Verilog, iVerilog, or Verilator) before the circuit description (e.g., at the behavioral level, data flow, or register transfer), here at Register Transfer Level (RTL) as a *.v file, is output.

    ![DesignEntry](Bilder5/DesignEntry.jpg)

- **Functional Verification:** It is important to identify design errors early on. Therefore, the hardware description of the circuit, e.g., in Verilog, must be promptly verified against the circuit requirements. This is done through simulation or formal methods, using both the previously generated circuit descriptions (at Register Transfer Level (RTL)) and the netlists generated in the following steps.
- **RTL Synthesis:** In this step, the HDL description of the logic circuit, at Register Transfer Level (RTL), i.e., the *.v file, is converted into a circuit consisting of logic cells, the so-called netlist.
    ...
    ![VerifizierungSynthese](Bilder5/VerifizierungSynthese.jpg)

- **Layout Synthesis:** During physical implementation, the netlist, i.e., the circuitry consisting of logic cells, is converted into a layout for photomask fabrication for the ASIC production in the cleanroom. \
This complex step comprises several substeps that are typically automated using a workflow. These include: floor planning, placement, clock tree synthesis, and routing. \
Because placement and routing are the most time-consuming processes, these steps are sometimes referred to collectively as "placement and routing," or PnR.
    ...
    ![Layout](Bilder5/Layout.jpg)

- **Signoff:** The signoff process ensures that the ASIC functions properly, operates efficiently, and ultimately delivers what it promises, …

    ![Signoff](Bilder5/Signoff1.jpg)

    … before the chip design is released for chip manufacturing.

    ![Signoff](Bilder5/Signoff2.jpg)

Please note that the five steps mentioned here are essential overarching points. There are numerous other design steps that are assigned to these or are applied systematically throughout the flow.


## The open-source infrastructure library LibreLane
...
- **_Design Entry:_** Designs are written using Hardware Description Languages ​​(HDL). Typical HDLs are:
...
- **_RTL Linting_** is a static analysis of the hardware design description code at Register Transfer Level (RTL) for

    * errors,
    * stylistic issues, and
    * compliance with design rules

- **_RTL Synthesis_** includes:

    * Verilog analysis and conversion with Yosys:
        * Yosys reads and analyzes your Verilog RTL files.
        * The tool converts the Verilog code into an internal representation that is used by the solver engines for further processing.

- **_Technology Mapping_** includes:
    * Logic optimization and mapping with ABC:
        * After analysis with Yosys, the code is passed to ABC.
        * ABC performs the downstream logic optimization and takes care of the technology mapping, i.e., translating the abstract logic into a concrete implementation with the cells of a specific technology.
    * Yosys Witness Framework:
        * Yosys offers a unified witness framework that generates cover traces and counterexamples.
        * This makes it possible to verify and debug the results of the synthesis and technology mapping.


### Backend
#### Layout Synthesis

- **_Floorplanning_**  is to prepare the circuit partitioning in blocks and the positioning so that each block created can be placed and wired internally and externally. Thus, the three main goals of floor planning are

    * shaping the circuit blocks
    * arrangement and positioning of the circuit blocks and
    * the signal assignment of the external I/O connections (pin assignment).

- **_Placement_** Based on a given synthesized circuit netlist together with a technology library, a valid placement layout for the circuit blocks is made. The layout is optimized according to the aforementioned objectives and ready for cell resizing and buffering.
There are usually multiple optimization objectives due to timing and signal integrity satisfaction.

    * *Total wire length:* the sum of the lengths of all the wires in the design
    * *Routing congestion:* local congestion is the difference between the lengths of wires in a region and the length of routing tracks available in that region; local values can be aggregated in several ways, such as adding up.
    * *Power:* dynamic switching power depends on wirelengths, which in turn depend on component locations.

- **_Clock Tree Synthesis (CTS)_** involves placing buffers and inverters along the clock paths, to reduce clock skew (the difference in clock arrival times), minimizes latency (delay in signal-transfer), and creates a stable clock tree. Key steps are:

    * *Clock Tree Construction:* designs the initial clock routes and selects an appropriate clock tree structure.
    * *Buffer and Gate Sizing:* adjusts the sizes of buffers and inverters to meet timing and power goals.
    * *HFN Synthesis:* uses high-fan-out buffers for stable clock distribution across large networks.
    * *Buffer Relocation:* moves buffers strategically to improve synchronization and reduce interference.

- **_Routing_** is to establish the desired electrical connections and thus realize the functionality of the circuit.
Typical routing phases are:

    * *Global routing:* where global connection paths for the signals are planned exactly, with focus on wire length, power consumption, and timing.
    * *Detailed routing:* is the final step of routing, in which concrete paths of the individual traces are calculated, with focus on design rules and complete separation of traces.

    Main goals and processes are:

    * *Optimizing interconnects:* Ensuring that signals are transmitted quickly and reliably between components.
    * *Compliant with design rules:* Considering complex rules that specify minimum spacing and geometries of metal traces on the chip to avoid manufacturing issues.
    * *Efficiency and performance:* Minimizing noise and propagation delays to achieve high chip performance.

-   **_RC Extraction_** As interconnect wires are near to each other, they provide besides their line resistance characteristic capacities, which are commonly all to minimize for a high design performance.

    Three types of capacitances appear in designs:

    * Coupling Capacitance Cc = ε*T/S
    * Area Capacitance per Unit length Ca = ε*W/H
    * Fringe Capacitance Cf: due to the non-uniform distribution of electric fields around the edges of conductive components.

    The values of the capacitances are dependent on: dielectric constants (ε), widths of metals (W), thicknesses of metals (T), spacing between the metals (S), thickness of dielectrics (ILD = interlayer dielectric) (H).

    The line resistance of metal layers depends on its materials and on its geometries.

    * Resistance per unit length = p*W/T

    which depends on resistivity of the metal layer (p), widths of metals (W) and  thicknesses of metals (T).


#### Spinoff / Verification

- **_Spinoff / Verification_** is the process of rigorously testing and validating an integrated circuit (IC) design to ensure, that ..

    * *Requirements:* .. it meets its functional, performance, and reliability specifications before manufacturing.
    * *Complexity:* Rising complexity in modern IC designs lead to numerous different errors. 
    * *Cost of Mistakes:* Manufacturing is extremely costly and time-consuming - so are "silicon respins"

    Key Concepts and Techniques are:

    * *Functional Verification:* This involves running test cases on a virtual model of the chip to confirm it performs its intended functions under all possible scenarios.
    * *Formal Verification:* This uses mathematical methods to formally prove the correctness of specific design aspects, which can catch corner cases that simulation might miss. 
    * *Simulation:* A common technique where test scenarios are run on a design model (like RTL) to detect functional errors. 
    * *Coverage Metrics:* These measure how thoroughly the design has been tested, indicating the percentage of code or functionality that has been covered by the tests. 
    * *Verification Intellectual Property (VIP):* Pre-built, reusable components and testbenches that accelerate the verification of common functionalities within a chip. 
    * *AI and Machine Learning:* Emerging applications of AI and ML are helping in areas like automated test generation and coverage analysis, identifying potential issues that human engineers might overlook.
...
Design Entry is the initial phase where engineers translate the functional requirements of a chip into a detailed, high-level design, typically using Hardware Description Languages (HDLs) such as Verilog or VHDL to describe the chip's logic and architecture at the Register Transfer Level (RTL).

- Defining Functionality: This is the stage where the chip's intended behavior, features, and specifications are defined and coded into a formal language.
- Hardware Description Languages (HDLs): Engineers use HDLs like Verilog or VHDL to describe the chip's logic and the flow of data between registers.
...
- Position within the Chipdesign-Flow: Design Entry is the initial phase where engineers translate the functional requirements of a chip into a detailed, high-level design, typically using Hardware Description Languages (HDLs).
...
Early design error detection: Linting finds errors (e.g., design flaws, synthesis-related errors, unwanted latches, incompatible bus widths, multi-drive ports, incorrect reset logic, incorrect use of clock signals, or style and readability issues) that even simulation might not detect.
...
- Design Constraints: Specifications for performance, power consumption, timing, and area are provided to the synthesis tool as design constraints for the design being created.
  - Power Constraints: Specifies the maximum power consumption of the design.
...
Technology mapping translates a technology-independent functional (generic) netlist-description (from an RTL synthesis) into a technology-specific implementation by finding the best way to realize the abstract logic using the available physical gates from a given library, which contains pre-defined, optimized logic gates from a particular manufacturing process. This, in order to produce a final, so called mapped netlist composed exclusively of gates from that library.
...
Chip design floorplanning uses Netlists, macro definitions, and I/O pin locations as inputs to determine the placement, shape, and orientation of large functional blocks and components (macros, IP cores, etc.) and I/O pads within the chip area. The primary outputs of floorplanning are a well-defined chip area, optimized placement of blocks and I/Os, and the resulting layout specifications that minimize chip area and interconnect length, thereby improving performance, power consumption, and facilitating later design steps like placement and routing. The key goals of floorplanning are:
...
The process includes in detail:
...
Chip-design placement is a crucial step in physical design that assigns exact locations for components like standard cells and macros on a 2D chip layout, optimizing for power, performance, and area (PPA) by minimizing wirelength, routing congestion, and placement density while meeting design constraints. It involves using automated tools to manage the complexity of billions of components, to ensure the final design is manufacturable and meets performance goals. Why Placement is Critical:
...
Types: Data inputs, control inputs, clock signals, reset signals, interrupt requests, bus or multiplex inputs.
...
Types: The signals or data output by the chip. These include result data, status signals, interrupt requests, or debug output.
...
In chip design's physical signoff, checks have become more complex as VLSI designs approach 22nm and below process nodes, because of the increased impact of previously ignored (or more crudely approximated) second-order effects. There are several categories of signoff checks.
